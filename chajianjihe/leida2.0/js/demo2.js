...
$(function(){
  ...
  //模拟数据   当前环数5
  var jsonstr="[{'now':100,'max':999},{'now':25,'max':100},{'now':50,'max':100},{'now':25,'max':100},{'now':99,'max':100}]";
  ...
  function staticBG(jsonarray){
    ...
    {
      grd.addColorStop(1/jsonarray.length*i,"hsl("+(1-(jsonarray[i].now/jsonarray[i].max).toFixed(2))*125+",100%,50%)");
    }
    ctx.fillStyle=grd;
    ctx.fill();
    ctx.strokeStyle="#fff";
...
//    2-3  文字添加函数
//
  function addText(jsonarray){
    //drawRoundedRect('rgba(255,255,255,0)',  'rgba(0,0,0,0.6)',  x/2-50,y/4, x, y/2, 40);
    ctx.font="10px Arial";
    ctx.fillStyle="#000";
    for(var i=0;i<jsonarray.length;i++)
    {
      ctx.fillText(i+1+"环",x/2-50+i*80,y/2-20-w+i*80);
      ctx.fillText("当前人数:"+jsonarray[i].now,x/2-50+i*80,y/2-20+i*80);
      ctx.fillText("最大容纳人数:"+jsonarray[i].max,x/2-50+i*80,y/2-20+w+i*80);
    }
    //保留一位小数
